...
// \todo unify import/export_shared
...
namespace
{
  auto export_shared_main (mcs::util::Args args) -> int
  {
    ...
    return mcs::util::ASIO::run
      ...
      , [&]<mcs::util::ASIO::is_protocol ProviderProtocol>
          ...
          )
        {
          ...
          using FileWrite = mcs::core::control::command::file::Write;

          auto errors {std::list<Error>{}};
          auto bytes_written {mcs::core::memory::make_size (0)};

          struct Collector
          {
            auto result ( mcs::iov_backend::Location const&
                        , FileWrite::Response bytes_written
                        ) noexcept
            {
            ...
            mcs::core::memory::Size* _bytes_written;
            std::list<Error>* _errors;
          };
          auto collector
            { Collector { std::addressof (bytes_written)
          ...
          mcs::rpc::multi_client::call_and_collect<FileWrite>
            ( [&] (auto const& location)
              {
                ...
                return FileWrite
                  ...
                  , location._parameter_file_write
          ...
          fmt::print
            ...
            , time_transfer
            , time_transfer
              . per_second (size_cast<std::size_t> (bytes_written)) / (1L << 20L)
            );
...
{
  return mcs::util::main (argc, argv, export_shared_main);
}
