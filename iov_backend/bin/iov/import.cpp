...
namespace
{
  auto import_main (mcs::util::Args args) -> int
  {
    ...
    auto const file_name {mcs::util::ExistingPath {args[5]}};
                            ...
                            )
                      {
                        auto const write_request_data
                          ...
                          . write ( { std::addressof (collection)
                                    ...
                                    , make_off_t
                                        ( filled_buffer->offset.collection
                                        )
                                    }
                                  )
                          . get()
             ...
             , [&]()
               {
                 error_collector.collect_errors_as<ReaderError>
                   ( [&]
                     {
                       auto const open_file {mcs::util::fopen (*file_name, "rb")};
                             ...
                             )
                       {
                         ...
                         auto const bytes_read
                           { mcs::util::syscall::pread
                             ( fd
                             , assigned_buffer->buffer.data()
                             , nbyte
                             , make_off_t (assigned_buffer->offset.file)
                             )
                           };

                         if (std::cmp_not_equal (bytes_read, nbyte))
                         {
                           throw std::runtime_error
                             { fmt::format
                                 ( "Error when reading from offset {}"
                                   " : Wanted {} bytes, got {} bytes"
                                 , assigned_buffer->offset.file
                                 , nbyte
                                 , bytes_read
                                 )
                             };
...
{
  return mcs::util::main (argc, argv, import_main);
}
