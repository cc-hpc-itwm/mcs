...
namespace
{
  auto segment_remove_main (mcs::util::Args args) -> int
  {
    if (args.size() != 5)
    {
      throw std::invalid_argument
        { fmt::format
            ( "usage: {} provider_connectable storage_id storage_parameter segment_id"
            , args[0]
            )
        };
    }
    ...
    auto const segment_id
      {mcs::util::read::read<mcs::core::storage::segment::ID> (args[4])};
    ...
    return mcs::util::ASIO::run
      ...
      , [&]<mcs::util::ASIO::is_protocol Protocol>
          (mcs::util::ASIO::Connectable<Protocol> connectable)
        {
          ...
          // Remove segment at the remote storages
          //
...
{
  return mcs::util::main (argc, argv, segment_remove_main);
}
