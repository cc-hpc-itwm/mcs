...
namespace mcs::util
{
  ...
  // when constructed with an existing path.
  //
  struct NonExistingPath
  {
    template<typename... Args>
      requires (std::is_constructible_v<std::filesystem::path, Args...>)
      NonExistingPath (Args&&...);

    struct Error
    {
      struct PathAlreadyExists : public mcs::Error
      {
      ...
      private:
        ...
        PathAlreadyExists (std::filesystem::path);
...
#include "detail/NonExistingPath.ipp"
